...
// GET /api/posts - generic listing (e.g. for admin/debug or a "discover" page)
...
const updatePost = async (req, res, next) => {
  try {
    ...
    // Ownership check: only the author (compared server-side via the
    // authenticated token) can edit - never trust an ID from the request body.
...
const deletePost = async (req, res, next) => {
  try {
    ...
    // Note: cascading delete of this post's comments/likes is added in
    // later stages once those controllers exist (Stage 6-7).
